      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-2">
        {featuredProjects.map((project, index) => (
          <motion.div
            ...
            className="group overflow-hidden rounded-[2.5rem] border border-cyan-300/30 bg-gradient-to-br from-cyan-400/10 via-blue-500/10 to-violet-500/10 p-6 backdrop-blur-xl md:p-8 transition hover:-translate-y-1"
          >
            <div className="flex flex-col gap-6">
              <div className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-black/20">
                {project.image ? (
                  <motion.img
                    src={project.image}
                    alt={project.title}
                    className="h-52 w-full object-cover rounded-[1.25rem]"
              ...
              <div>
                <div className="flex flex-wrap items-center gap-3">
                  <p className="text-sm text-cyan-200/90">{project.category}</p>
                  {project.status && (
